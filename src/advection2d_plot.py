...
# u0
axes[0, 0].imshow(np.flipud(square2d(x[(t == 0).squeeze()]).numpy().reshape(n,n, order='F')), cmap='hot', vmin=hmin, vmax=hmax, interpolation='nearest', extent = bounds.flatten())
axes[0, 0].set_title('u0')
# each subplot is a snapshot in time
for i in range(len(tplot)):
    idx = (t == tplot[i]).squeeze()
    j = i + 1
    im = axes[j // 3, j % 3].imshow(np.flipud(fx[idx].numpy().reshape(n,n, order='F')), cmap='hot', vmin=hmin, vmax=hmax, interpolation='nearest', extent = bounds.flatten())
