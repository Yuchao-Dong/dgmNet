...
def random_sample(
    n: int = 10,
    d: int = 1,
    subdomain: str = "interior",
    T: int = 1,
    bounds: torch.Tensor = None,
    bounded_dims: list = None,
):
    """
    args:
...
def regular_sample(
    ...
    combinations = (
        torch.cartesian_prod(*([X[..., i] for i in range(d)])) if d > 1 else X
    )
...
def slotted_disk(x: torch.Tensor, r: float = 1, slotw: float = 0.3, slotl: float = 1.5):
    """
    args:
    ...
    slot_mask = torch.logical_and(torch.abs(x[:, 1]) < slotw / 2, x[:, 0] > r - slotw)
...
class u0Presets:
    ...
    def square(self, x):
        return torch.where(x > 0.3, torch.where(x < 0.7, 1.0, 0.0), 0.0)
